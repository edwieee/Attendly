...
export default function RefinedMarkAttendanceWithSubject() {
  ...
  const handleSave = async () => {
    if (isStudentView) return; // Block save if student tries to bypass UI
  ...
  return (
    ...
    >
      <div className="p-8 max-w-7xl mx-auto lg:px-12">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 sticky top-16 lg:top-0 bg-white/95 backdrop-blur-sm py-4 lg:py-6 z-30 border-b border-slate-50 gap-4 sm:gap-0">
          <div>
            <h2 className="text-2xl sm:text-3xl font-extrabold font-headline tracking-tight text-on-surface">
              {isStudentView ? 'Daily Attendance Registry' : 'Mark Attendance'}
            </h2>
        ...
        <section className="bg-surface-container rounded-xl p-5 sm:p-6 mb-8 flex flex-col sm:flex-row flex-wrap gap-4 sm:gap-6 items-start sm:items-end">
          <div className="flex flex-col gap-2 w-full sm:w-56">
            ...
            <div className="relative group">
              ...
              <input
                className="bg-surface-container-lowest border-none rounded-lg pl-10 pr-4 py-3 text-sm font-medium shadow-sm focus:ring-2 focus:ring-primary w-full disabled:opacity-50"
                type="date"
                value={selectedDate}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-surface-container-low/50">
                  ...
                  <th className="px-8 py-4 text-xs font-bold text-on-surface-variant uppercase tracking-widest font-label text-right">Status</th>
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={student.id} className="group hover:bg-surface-container-low/30 transition-colors">
                      ...
                      <td className="px-8 py-5 text-right">
                        <button
                          onClick={() => !isStudentView && setAttendance(prev => ({...prev, [student.id]: !prev[student.id]}))}
                          className={`inline-flex items-center gap-2 px-5 py-2 rounded-full text-xs font-bold shadow-sm transition-all duration-200 ${
                            isStudentView ? 'cursor-default' : 'cursor-pointer active:scale-95'
                          } ${
                            isPresent
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12 pb-12">
          <div className="bg-primary-container p-6 rounded-2xl relative overflow-hidden group">
            <div className="relative z-10">
              ...
              <div className="mt-4 flex items-center gap-2 text-on-primary-container/80 text-sm">
                ...
                <span>{attendanceRate}% average rate</span>
              </div>
            </div>
          </div>
          <div className="bg-error-container p-6 rounded-2xl relative overflow-hidden group">
            <div className="relative z-10">
              ...
              <div className="mt-4 flex items-center gap-2 text-on-error-container/80 text-sm">
                ...
                <span>{absentCount || '0'} recorded today</span>
              </div>
            </div>
          </div>
        </section>
      ...
      <div className={`fixed bottom-8 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-6 py-4 rounded-xl shadow-2xl flex items-center gap-4 z-50 transform transition-all duration-300 ${showToast ? 'translate-y-0 opacity-100' : 'translate-y-24 opacity-0 pointer-events-none'}`}>
        <span className="material-symbols-outlined text-primary">check_circle</span>
        <span className="text-sm font-medium whitespace-nowrap">Dashboard context synchronized.</span>
      </div>
    </motion.div>
  );
}